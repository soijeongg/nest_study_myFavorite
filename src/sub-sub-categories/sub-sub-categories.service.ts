...
export class SubSubCategoriesService {
  constructor(
    @InjectRepository(SubSubCategory) private subSubCategoryRepository: Repository<SubSubCategory>,
    private subCategoriesService: SubCategoriesService,
  ) {}

  //TODO: 카테고리, 서브카테고리 검사하기
  async create(createSubSubCategoryDto: CreateSubSubCategoryDto, categoryId: number, subCategoryId: number, status) {
    if(status != userType.ADMIN) {
      throw new HttpException('권한이 없습니다', HttpStatus.BAD_REQUEST);
    }
    let {subSubCategoryName}  = createSubSubCategoryDto
   const SubCategory = await this.subCategoriesService.findOne(categoryId, subCategoryId)
   if(!SubCategory) {
    throw new HttpException('해당하는 서브카테고리가 없습니다', HttpStatus.NOT_FOUND)
   }
   //이름 검사하기
   ...
   if(checkName){
    throw new HttpException('이미 존재하는 서서브 카테고리 입니다', HttpStatus.BAD_REQUEST)
   }
   const subCategories = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId)
const subSub = await this.subSubCategoryRepository.create({
  subSubCategoryName, subCategory: subCategories
})
...
//TODO: 카테고리, 서브카테고리, 서서브 카테고리를 받아 서서브 카테고리와 최애 전체를 반환한다
  async findAll(categoryId: number, subCategoryId: number) {
    const subCategory = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId)
    if(!subCategory) {
     throw new HttpException('해당하는 서브카테고리가 없습니다', HttpStatus.NOT_FOUND)
    }
    //있다면 이걸로 찾기 
...
//TODO: 하나의 서서브 카테고리를 찾는다
  async findOne(categoryId: number, subCategoryId: number, subSubCategoryId: number) {
    //서브카테고리, 카테고리를 검사한다
    const subCategory = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId)
    if(!subCategory) {
     throw new HttpException('해당하는 서브카테고리가 없습니다', HttpStatus.NOT_FOUND)
    ...
    const findOneSub = await this.subSubCategoryRepository.findOne({
      where:{subSubCategoryId, subCategory, deleteAt: null}
    }) 
    if(!findOneSub) {
      throw new HttpException('해당하는 서서브 카테고리가 없습니다', HttpStatus.NOT_FOUND)
    }
    return {
...
//TODO: 업데이트 한다  카테고리,서브카테고리, 서서브커테고리를 검사하고 이름을 검사한다
 async  update(categoryId: number, subCategoryId: number, subSubCategoryId: number,updateSubSubCategoryDto: UpdateSubSubCategoryDto,status: string ) {
  if(status != userType.ADMIN) {
    throw new HttpException('권한이 없습니다', HttpStatus.BAD_REQUEST);
  }
  let {subSubCategoryName}  = updateSubSubCategoryDto
    const subCategory = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId)
    if(!subCategory) {
      throw new HttpException('해당하는 서브카테고리가 없습니다', HttpStatus.NOT_FOUND);
     }
  ...
  async remove(categoryId: number, subCategoryId: number, subSubCategoryId: number, status: string) {
    if(status != userType.ADMIN) {
      throw new HttpException('권한이 없습니다', HttpStatus.BAD_REQUEST);
    }
      const subCategory = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId)
  ...
  async findOneSubCategory(categoryId: number, subCategoryId: number,subSubCategoryId: number) {
    const category = await this.subCategoriesService.findOneSubCategory(categoryId, subCategoryId);
    if (!category) {
